...
{ Attribute src; 
  ...
  public AttributeMatching(Expression source, Attribute target, Attribute var, 
                           Vector auxvars) 
  { src = new Attribute("" + source, new Type("String",null), 
                        ModelElement.INTERNAL); // Why String? Better as source.getType()
  ...
  { if (v.getName().equals(srcname)) 
    ...
    if (srcvalue != null) 
    { if (srcvalue.hasVariable(v.getName()))
      { return false; }
    }  
  ...
  { Vector res = new Vector(); 
    ...
    { if (srcvalue instanceof SetExpression) 
      { SetExpression sse = (SetExpression) srcvalue; 
        ...
        { // Sequence{r1,r2} --> trg inverts to trg->at(1) --> r1, trg->at(2) --> r2
  ...
  static Expression dataConversion(Expression sexp, Type stype, Type ttype)
  ...
  { Vector res = new Vector(); 
    ...
    { Vector trgents = trg.intermediateEntities(); 
      ...
      { Entity trgent = (Entity) trgents.get(k); 
        ...
        { System.out.println(">!! Warning: intermediate target class " + trgent + " in " + trg + 
          ...
          String ans = 
              JOptionPane.showInputDialog("Replace " + trgent + " by?:");
          if (ans != null) 
    ...
    if (("" + srct).equals("" + trgt)) { } 
    else if (Type.isRecursiveSubtype(srct,trgt))
    { CorrelationPattern q = new CorrelationPattern("Widen type", 
                                   "Feature " + realsrc + "::" + src +
                                   " type widened from " + srct + " to " + trgt + " in " +  
                                   realtrg + "::" + trg); 
    ...
    else if (Type.isRecursiveSubtype(trgt,srct))
    { CorrelationPattern q = new CorrelationPattern("Narrow type", 
                                   "Feature " + realsrc + "::" + src +
                                   " type narrowed from " + srct + " to " + trgt + " in " +  
                                   realtrg + "::" + trg); 
      q.addSourceEntity(realsrc); 
      q.addTargetEntity(realtrg); 
      q.addSourceFeature(src); 
    ...
    else if (mult1 == ModelElement.MANY && src.getUpper() == 1 && mult2 == ModelElement.MANY) 
    { CorrelationPattern q = new CorrelationPattern("Widen multiplicity", 
                                   "Feature " + realsrc + "::" + src +
    ...
    else if (mult1 == ModelElement.MANY && mult2 == ModelElement.MANY && trg.getUpper() == 1) 
    { CorrelationPattern q = new CorrelationPattern("Narrow multiplicity", 
                                   "Feature " + realsrc + "::" + src +
  ...
  { // nme = srcdata at the end of the path
    ...
    { Attribute p = (Attribute) path.get(0); 
      ...
      { valueExpC = AttributeMatching.dataConversion(valueExp, src.getType(), p.getType()); } 
  ...
  { Type ttarg = trg.getType(); 
    ...
    { Attribute p = (Attribute) path.get(0); 
      ...
      { BasicExpression sexp = new BasicExpression(src); 
        Expression texp = AttributeMatching.dataConversion(sexp,tsrc,ttarg); 
  ...
  { if (path.size() == 1) 
    { Attribute p = (Attribute) path.get(0); 
      ...
      { BasicExpression sexp = new BasicExpression(evar); 
        Expression texp = AttributeMatching.dataConversion(sexp,tsrc,t); 
      } 
  ...
  { Type ttarg = trg.getType(); 
    ...
    { Attribute p = (Attribute) path.get(0); 
      ...
      { BasicExpression srcexp = (BasicExpression) svar; 
        ...
        Expression sconv = AttributeMatching.dataConversion(srcexp, src.getType(), t);  
        res = tvar + "." + p + " := " + sconv + ";"; 
  ...
  { Expression sexp = new BasicExpression(svar + "." + src);
    ...
    if ("String".equals(trg.getType() + "") && src.getType().isEnumeration())
    { vexp = AttributeMatching.dataConversion(sexp, src.getType(), trg.getType()); } 
    else if ("String".equals(src.getType() + "") && trg.getType().isEnumeration())
    { vexp = AttributeMatching.dataConversion(sexp, src.getType(), trg.getType()); } 
  ...
  { Vector path = trg.getNavigation(); 
    ...
    { System.out.println(">>> Additional assignment " + trgvar + "." + trg + " <- " + expr); 
      Attribute trgref = trg.objectReference(); 
      if (trgref.isMultiValued())
      { Attribute f = trg.getFinalFeature(); 
        newdo.add("for (_x in " + trgvar + "." + trgref + ") { _x." + f + " <- " + expr + "; }"); 
      }  
      else 
...
{ if (path.size() == 0)
  ...
  { System.err.println("!! Null element type in " + sexpr); 
    ...
    sx = Identifier.nextIdentifier("var$"); 
  ...
  { if (p1.isSingleValued())
    ...
    { if (sexpr.isMultipleValued() && srcent != null && 
      ...
      else if (sexpr.isMultipleValued() && srcent != null)
      { Binding sexp2 = atlTargetMap(nextatt,pathtail,new BasicExpression(srcvar),
        ...
        { MatchedRule mr = new MatchedRule(true,true);
          ...
          ope.addBinding(sexp2); 
          // implementedBy.put(preattname, ope);
          implementedBy.put(preattname, p1x); 
          op.setElement(ope);
          mr.setInPattern(ip);  
          mr.setOutPattern(op);
          newrules.add(mr); 
