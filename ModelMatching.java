...
{ Map mymap; 
  ...
  final static double ENUMSTRING = 0.5; 
  final static double STRINGENUM = 0.5; 
  final static double BOOLENUM = 0.8;  // boolean matching to an enum

  final static double OPTONE = 0.8; 
  final static double ONEOPT = 0.8; 
  final static double SETSEQUENCE = 0.6; 
  final static double SEQUENCESET = 0.6; 
  ...
  { Map invmm = Map.inverse(mymap); 
    ...
    { EntityMatching em = (EntityMatching) entitymatches.get(i); 
      EntityMatching eminv = em.invert(); 
  ...
  public void copySuperclassMatchings()
  { for (int i = 0; i < entitymatches.size(); i++) 
    { EntityMatching em = (EntityMatching) entitymatches.get(i); 
      ...
      { em.copyAttributeMappingsToSubclasses(srcsubs,entitymatches); } 
    } 
  } 
  ...
  public String qvtTransformation(Vector types)
  ...
  public String umlrsdsTransformation()
  { String res = ""; 
  ...
  public String qvtoTransformation(Vector types)
  { String res = "transformation tau(in : src, out : trg);\n\n"; 

    for (int i = 0; i < types.size(); i++) 
    { Type et = (Type) types.get(i); 
      if (et.isEnumeration())
      { res = res + "  " + et.enumQueryOpsQVTO() + "\n\n"; } 
    }     
