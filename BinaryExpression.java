...
{ // Represents binary expressions:   left operator right
  ...
  { operator = op; 
    ...
    else if ("->one".equals(op)) { operator = "->exists1"; } 
    else if ("div".equals(op)) { operator = "/"; } 
  ...
  { if ("or".equals(operator))
    ...
    /* if (":".equals(operator) ||
        ...
        "->hasSuffix".equals(operator) || "/<:".equals(operator))
  ...
  { String basicString = 
    ...
    { basicString = left + "->excluding(" + keyValue + ", " + right + ")"; } 
             ...
             operator.equals("->excluding") || operator.equals("->includes") ||
             ...
             operator.equals("->excludesValue") || operator.equals("->restrict") ||  
             operator.equals("->selectMinimals") || operator.equals("->union") ||
             operator.equals("->intersectAll") || operator.equals("->unionAll") ||
             operator.equals("->count") || operator.equals("->indexOf") || 
             operator.equals("->before") || operator.equals("->after") || 
  ...
  { BinaryExpression res = (BinaryExpression) clone(); 
    ...
    if ("->select".equals(operator) || "->reject".equals(operator) || 
        ...
        "->unionAll".equals(operator) || "->intersectAll".equals(operator) ||                         "->concatenateAll".equals(operator) || "->existsLC".equals(operator) || 
        "->exists".equals(operator) || "->exists1".equals(operator) || 
  ...
  { if (operator.equals("->count") || operator.equals("->indexOf") || 
        ...
        operator.equals("!=") || operator.equals("#") || operator.equals("#LC") || 
        operator.equals("/<:") ||
...
{ if (this.syntacticComplexity() < 10) { return; }
  ...
  { Vector context = new Vector(); 
    ...
    else if (operator.equals("->select") || operator.equals("->selectMaximals") ||
             operator.equals("->selectMinimals") || operator.equals("->sortedBy") ||  
    ...
    { // System.out.println("Type-checking select/reject " + this); 
      ...
      return true; 
             ...
             operator.equals("->intersectAll"))   // and |C
    { // System.out.println("Type-checking collect " + this); 
      ...
      { System.err.println("!!! TYPE ERROR: no element type in " + left + " in " + this); 
        JOptionPane.showMessageDialog(null, "no element type for " + left + " in " + this, 
                                      "Type error", JOptionPane.ERROR_MESSAGE);
      } 
    ...
    if ("->count".equals(operator) || "->indexOf".equals(operator))
    ...
    { if ("String".equals(left.type + ""))
      ...
      { type = left.elementType;
        ...
        { System.err.println("ERROR: no element type in " + this + " " + env); 
    ...
    { tcMinus(tleft,tright,eleft,eright); }
             ...
             operator.equals("->excluding") || operator.equals("->prepend") || 
             operator.equals("->intersection") || operator.equals("/\\"))
    ...
    // System.out.println("Type checked: " + this + " Type is: " + type);
    if (type == null)
    { JOptionPane.showMessageDialog(null, "No type found for " + this, 
...
// rolea = roleb ?? one multiple other not?
  ...
  { if (tleft != null && tright != null)
    { String tlname = tleft.getName();
      ...
      { System.err.println("TYPE ERROR: disallowed types in +: " + this);
  ...
  { // union and intersection
    ...
    { if (operator.equals("^") || operator.equals("->prepend") || operator.equals("->append"))
      { if ("Sequence".equals(tlname)) { } 
        ...
        { System.err.println("WARNING: ^, prepend, append must be applied to sequence: " + this); } 
        type = new Type("Sequence",null); 
      } 
      else if (operator.equals("->excluding") || operator.equals("->including") ||
               operator.equals("->intersection") || operator.equals("/\\"))
      { if ("Sequence".equals(tlname))
        { type = new Type("Sequence",null); }  
        else 
    ...
    { if (tleft.equals(tright) && tlname.equals("Set")) 
      { type = new Type("Set",null); }
      else if (tleft.equals(tright) && tlname.equals("Sequence")) 
      { type = new Type("Sequence",null); }
      else 
      { System.out.println("TYPE ERROR: invalid types " + tleft + " " + tright + " in: " + this);
        JOptionPane.showMessageDialog(null, "Disallowed types in " + this, 
                                      "Type error", JOptionPane.ERROR_MESSAGE);
        type = new Type("Set", null);
      }
    ...
    { System.err.println("TYPE ERROR: missing types in: " + this);
      if (tright == null && tleft != null)
      { right.setType(tleft);
        type = new Type("Set",null);
      }
    ...
    if (operator.equals("->intersection") || operator.equals("/\\") ||
        operator.equals("->excluding"))
    { elementType = etleft;
      type.setElementType(elementType); 
    } 
  ...
  { // left should be set or sequence, right a boolean
  ...
  { // left should be set or sequence, right an expression
    ...
    { if (right.elementType == null) 
      ...
      { type = new Type("Set",null); }     // Always unordered
      // entity = eright; 
    } // ?? 
...
{ if (op.equals("="))
  ...
  { String res;
    ...
    if (operator.equals("|C") || operator.equals("->collect"))  // and |C
  ...
  { String res;
    ...
    { String rw = rqf; 
      ...
      { res = "SystemTypes.concatenate(" + rss + "," + lqf + ")"; }
      else 
  ...
  { // collect_ind(lqf) where ind is a unique index for left and right
    ...
    { if (left.elementType == null) 
      { System.err.println("DESIGN ERROR: no element type for: " + left);                 JOptionPane.showMessageDialog(null, "no element type for " + left + " in " + this, 
                                      "Design error", JOptionPane.ERROR_MESSAGE);
      } 
      else
      { localentity = left.elementType.getEntity(); }  
...
{ String val2;
  ...
  { Statement ufr = right.statLC(env,local);
    if ("true".equals(left + ""))
    { return ufr; }
    IfStatement istat = new IfStatement(left,ufr);
    return istat;
  }
  ...
  { BasicExpression lbe = (BasicExpression) left;
    ...
    { Statement upd = lbe.generateDesignSubset(right);
      ...
      { Statement skip = new SequenceStatement();
        ...
        return new IfStatement(this, skip, upd);
      }
      return upd;
    } // but ->forAll in fact
    else if (operator.equals("->excludesAll"))
  ...
  { BasicExpression rbe = (BasicExpression) right;
    ...
    { Statement upd = rbe.generateDesignSubset(left);
      ...
      { Statement skip = new SequenceStatement();
        upd.setBrackets(true);
        return new IfStatement(test, skip, upd);
      }
      return upd;
    } // but ->forAll in fact
    else if (operator.equals("/<:"))
...
{ BinaryExpression erange = new BinaryExpression(":", evar, eset); 
  ...
  { String val2;
    ...
    return new InvocationStatement("/* No update form for: " + this + " */");
  } 
  ...
  { String val2;
    ...
    { val2 = right.queryFormJava6(env,local);
      return left.updateForm("Java6",env,((UnaryExpression) left).operator,val2,right,local);
    }
    else if (operator.equals("=") && left instanceof BinaryExpression)
    { val2 = right.queryFormJava6(env,local);
      return left.updateForm("Java6",env,((BinaryExpression) left).operator,val2,right,local);
    }
    else if (operator.equals("->includesAll") && left instanceof BasicExpression)
    { val2 = right.queryFormJava6(env,local);
  ...
  { String val2;
    ...
    { val2 = right.queryFormJava7(env,local);
      return left.updateForm("Java7",env,((UnaryExpression) left).operator,val2,right,local);
    }
    else if (operator.equals("=") && left instanceof BinaryExpression)
    { val2 = right.queryFormJava7(env,local);
      return left.updateForm("Java7",env,((BinaryExpression) left).operator,val2,right,local);
    }
    else if (operator.equals("->includesAll") && left instanceof BasicExpression)
    { val2 = right.queryFormJava7(env,local);
  ...
  { String val2;
    ...
    { val2 = right.queryFormCSharp(env,local);
      return left.updateForm("CSharp",env,((UnaryExpression) left).operator,val2,right,local);
    }
    else if (operator.equals("=") && left instanceof BinaryExpression)
    { val2 = right.queryFormCSharp(env,local);
      return left.updateForm("CSharp",env,((BinaryExpression) left).operator,val2,right,local);
    }
    else if (operator.equals("->includesAll") && left instanceof BasicExpression)
    { val2 = right.queryFormCSharp(env,local);
